...
module.exports = {
  ...
  var songInfo = video
    ...
    if (serverQueue) {
      ...
      .setColor("BLUE")
      .setDescription(`Queued: [${song.title}](${song.url}) [<@!${song.req.id}>]`)
   ...
   const queueConstruct = {
    ...
    if (!song){
      if (!online.afk) {
        sendError("If you want the bot stay 24/7 run `k!afk` command", message.channel)
...
let stream = null;  
    ...
    queue.connection.on("disconnect", () => message.client.queue.delete(message.guild.id));
      ...
      let thing = new MessageEmbed()
      .setColor("BLUE")
      .setDescription(`Started playing: [${song.title}](${song.url}) [<@!${song.req.id}>]`)
